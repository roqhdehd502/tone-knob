...
export class SubscriptionSvcController {
  ...
  async subscribe(@Payload() data: { userId: string; plan: string; externalPaymentId?: string }) {
    return this.subscriptionService.subscribe(data.userId, data.plan as any, data.externalPaymentId);
  }
