...
export class MediaProxyController {
  ...
  @ApiOperation({
    ...
    description:
      ...
      "저장 경로는 항상 본인 소유 네임스페이스(`users/{내 id}/...`) 하위로 강제되며, category로 하위 분류만 가능합니다(다른 사용자 경로를 지정할 수 없음). " +
    ...
    schema: {
      ...
      properties: {
        ...
        category: {
          type: "string",
          description:
            '하위 분류 (영문/숫자/대시/밑줄만 허용, 예: "audio-extract"). 미입력 시 users/{내 id} 바로 아래에 저장',
        },
      },
      required: ["file"],
    },
  ...
  @ApiResponse({
    status: 201,
    description: "{ url, path } — 업로드된 파일의 공개 URL과 Storage 경로 반환",
  })
  ...
  async uploadFile(
    ...
    // 보안: folder를 클라이언트가 직접 지정하게 하면 다른 사용자 네임스페이스에 쓸 수 있다(IDOR).
    // 항상 본인 소유 경로(users/{내 id})를 서버에서 강제하고, category는 영문/숫자/대시/밑줄만 허용해 하위 분류로만 사용한다.
    const safeCategory = category?.replace(/[^a-zA-Z0-9_-]/g, "").slice(0, 50);
    const folder = safeCategory ? `users/${user.id}/${safeCategory}` : `users/${user.id}`;
  ...
  @Post("cdn/to-cdn-url")
  @ApiOperation({
    summary: "Origin URL → CDN URL 변환",
    description:
      "Storage 원본 URL을 CDN 도메인 기준 URL로 변환합니다. CDN_BASE_URL 미설정 시 원본 URL을 그대로 반환합니다.",
  })
  @ApiResponse({ status: 200, description: "변환된 CDN URL 반환" })
  async toCdnUrl(@Body() body: ToCdnUrlDto) {
    return firstValueFrom(this.mediaClient.send("media.cdn.toCdnUrl", body));
  }

  @Post("cdn/to-origin-url")
  @ApiOperation({
    summary: "CDN URL → Origin URL 변환",
    description: "CDN 도메인 기준 URL을 Storage 원본 URL로 역변환합니다.",
  })
  @ApiResponse({ status: 200, description: "변환된 원본 URL 반환" })
  async toOriginUrl(@Body() body: ToOriginUrlDto) {
    return firstValueFrom(this.mediaClient.send("media.cdn.toOriginUrl", body));
  }

  @Post("cdn/signed-url")
  @ApiOperation({
    summary: "서명된 임시 URL 발급",
    description:
      "비공개 파일에 대해 일정 시간 동안만 유효한 서명된 URL을 발급합니다(녹음 파일 등 비공개 콘텐츠 공유용).",
  })
  @ApiResponse({ status: 200, description: "{ url: string } 반환" })
  async getSignedUrl(@Body() body: GetSignedUrlDto) {
    const url = await firstValueFrom(this.mediaClient.send("media.cdn.getSignedUrl", body));
    return { url };
  }

  @Get("cdn/status")
  @ApiOperation({
    summary: "CDN 연동 상태 조회",
    description: "CDN_BASE_URL 등 CDN 관련 환경변수 설정 여부를 조회합니다(헬스체크/디버깅용).",
  })
  @ApiResponse({ status: 200, description: "CDN 설정 상태 반환" })
  async getCdnStatus() {
    return firstValueFrom(this.mediaClient.send("media.cdn.status", {}));
  }
  ...
  @Get("regions")
  @ApiOperation({
    summary: "전체 미디어 리전 목록 조회",
    description:
      "합주(WebRTC)/스토리지 등에 사용 가능한 지역(리전) 목록과 각 리전의 헬스 상태를 조회합니다.",
  })
  @ApiResponse({ status: 200, description: "리전 목록 반환" })
  async getAllRegions() {
    return firstValueFrom(this.mediaClient.send("media.region.getAll", {}));
  }
  ...
  @ApiOperation({
    summary: "최적 리전 자동 선택",
    description:
      "클라이언트의 위도/경도를 기준으로 가장 가까운(지연시간이 낮을 것으로 예상되는) 헬시 리전을 추천합니다. 좌표 미전달 시 기본 리전을 반환합니다.",
  })
  ...
  @ApiOperation({
    summary: "리전 헬스체크 실행",
    description:
      "등록된 모든 리전에 헬스체크를 수행하고 최신 상태(healthy, latencyMs)를 반환합니다.",
  })
  @ApiResponse({ status: 200, description: "갱신된 리전별 헬스 상태 목록 반환" })
  async checkHealth() {
    return firstValueFrom(this.mediaClient.send("media.region.checkHealth", {}));
  }
}
