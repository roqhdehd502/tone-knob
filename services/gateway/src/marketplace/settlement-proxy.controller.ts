import { Controller, Get, Inject, Post, Query, UseFilters, UseGuards } from "@nestjs/common";
...
export class SettlementProxyController {
  ...
  @ApiOperation({
    summary: "정산 신청",
    description:
      "판매자가 누적된 미정산 매출에 대한 정산을 신청합니다. 실제 지급은 매월 자동 실행되는 정산 스케줄러를 통해 처리됩니다.",
  })
  ...
  async requestSettlement(@CurrentUser() user: RequestUser) {
    return firstValueFrom(this.marketplaceClient.send("settlement.request", { sellerId: user.id }));
  }

  @Get("my")
  @ApiOperation({
    summary: "내 정산 내역 조회",
    description: "판매자 본인의 정산 신청/지급 내역을 최신순으로 조회합니다.",
  })
  ...
  @ApiOperation({
    summary: "정산 요약 조회",
    description: "판매자의 총 매출, 수수료, 지급 완료액, 미정산(대기) 금액을 요약해 조회합니다.",
  })
  @ApiResponse({
    status: 200,
    description: "{ totalRevenue, totalFees, totalPaid, pendingAmount } 반환",
  })
  async getSummary(@CurrentUser() user: RequestUser) {
    return firstValueFrom(this.marketplaceClient.send("settlement.summary", { sellerId: user.id }));
  }
}
