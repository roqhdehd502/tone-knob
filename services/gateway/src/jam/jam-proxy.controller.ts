...
export class JamProxyController {
  ...
  @ApiBearerAuth()
  @ApiOperation({ summary: '합주방 생성', description: '로그인한 사용자를 호스트로 하는 합주방을 생성합니다. 실시간 합주는 별도 WebSocket(/jam, jam-svc HTTP 포트)으로 연결합니다.' })
  ...
  @Get()
  @ApiOperation({ summary: '합주방 목록 조회', description: '인증 없이 누구나 조회 가능합니다. 비공개방(isPrivate=true)도 목록에는 노출되지만 입장 시 비밀번호가 필요합니다.' })
  ...
  @Get(':id')
  @ApiOperation({ summary: '합주방 상세 조회', description: '방 정보(이름, BPM, 최대 인원, 현재 인원, 비공개 여부 등)를 조회합니다.' })
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: '합주방 참가', description: '방 정원이 가득 찼거나 비공개방 비밀번호가 틀리면 실패합니다. 참가 성공 시 Knob이 자동 적립됩니다. 참가 후 실시간 통신은 WebSocket으로 별도 연결해야 합니다.' })
  @ApiParam({ name: 'id', description: '합주방 ID' })
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: '합주방 나가기', description: '호스트가 나가도 방은 유지되며, 마지막 참가자가 나가면 방이 자동으로 비활성화됩니다.' })
  @ApiParam({ name: 'id', description: '합주방 ID' })
  ...
  @Get(':id/participants')
  @ApiOperation({ summary: '합주방 참가자 목록', description: '현재 방에 참가 중인 사용자 목록을 조회합니다.' })
  @ApiParam({ name: 'id', description: '합주방 ID' })
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: '합주방 닫기', description: '호스트만 가능합니다. 모든 참가자가 강제로 퇴장되고 방이 비활성화됩니다.' })
  @ApiParam({ name: 'id', description: '합주방 ID' })
