...
export class SubscriptionProxyController {
  ...
  @Get("plans")
  @ApiOperation({ summary: "구독 플랜 목록 조회", description: "free/premium/pro 플랜별 월 가격과 제공 기능을 조회합니다. 인증이 필요하지 않습니다." })
  ...
  @ApiBearerAuth()
  @ApiOperation({ summary: "현재 구독 정보 조회", description: "로그인한 사용자의 현재 활성 구독(플랜, 시작/만료일)을 조회합니다. 구독 중이 아니면 null을 반환합니다." })
  ...
  @ApiOperation({
    summary: "구독 신청",
    description: "premium/pro 플랜을 신규 구독하거나 변경합니다. PG사 결제가 선행되어야 하며 externalPaymentId로 결제 승인 키를 전달합니다.",
  })
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: "구독 취소", description: "현재 활성 구독을 즉시 취소합니다. 환불은 별도로 처리되지 않으며(/api/payments/:id/refund 사용), 다음 결제 주기부터 free 플랜으로 전환됩니다." })
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: "구독 히스토리 조회", description: "과거 구독(취소/만료 포함)을 포함한 전체 구독 이력을 최신순으로 조회합니다." })
  @ApiQuery({
    name: "page",
    required: false,
    type: Number,
  ...
  @UseGuards(JwtAuthGuard)
  @ApiBearerAuth()
  @ApiOperation({ summary: "타브 생성 제한 확인 (무료 플랜 월 3회)", description: "free 플랜은 월 3회까지 타브를 생성할 수 있습니다. premium/pro는 무제한입니다. 타브 생성 폼 진입 시 사전 확인용으로 사용합니다." })
