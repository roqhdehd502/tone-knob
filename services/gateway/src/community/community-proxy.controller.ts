...
export class CommunityProxyController {
  ...
  @ApiOperation({
    summary: "타브 좋아요 토글",
    description: "이미 좋아요를 눌렀으면 취소되고, 누르지 않았으면 추가됩니다(토글 방식).",
  })
  @ApiParam({ name: "tabId", description: "타브 ID" })
  @ApiResponse({ status: 200, description: "토글 후 좋아요 상태 및 총 좋아요 수 반환" })
  async toggleLike(@Param("tabId", ParseUUIDPipe) tabId: string, @CurrentUser() user: RequestUser) {
  ...
  @ApiOperation({
    summary: "내 좋아요 여부 조회",
    description: "로그인한 사용자가 해당 타브에 좋아요를 눌렀는지 조회합니다.",
  })
  @ApiParam({ name: "tabId", description: "타브 ID" })
  @ApiResponse({ status: 200, description: "{ liked: boolean } 반환" })
  async isLiked(@Param("tabId", ParseUUIDPipe) tabId: string, @CurrentUser() user: RequestUser) {
  ...
  @ApiOperation({
    summary: "댓글 작성",
    description: "parentId를 전달하면 해당 댓글의 대댓글로 등록됩니다.",
  })
  @ApiParam({ name: "tabId", description: "타브 ID" })
  ...
  @ApiOperation({
    summary: "댓글 목록 조회",
    description:
      "해당 타브의 최상위 댓글 목록을 조회합니다(대댓글은 /comments/:commentId/replies로 별도 조회).",
  })
  @ApiParam({ name: "tabId", description: "타브 ID" })
  ...
  async getReplies(@Param("commentId", ParseUUIDPipe) commentId: string) {
    return firstValueFrom(this.communityClient.send("community.getReplies", { commentId }));
  }
  ...
  @ApiOperation({
    summary: "댓글 삭제",
    description:
      "작성자 본인만 삭제할 수 있습니다. 대댓글이 있는 댓글을 삭제하면 대댓글도 함께 삭제됩니다.",
  })
  @ApiParam({ name: "commentId", description: "댓글 ID" })
  ...
  @ApiOperation({
    summary: "팔로우 토글",
    description:
      "이미 팔로우 중이면 취소되고, 아니면 팔로우됩니다(토글 방식). 본인을 팔로우할 수 없습니다.",
  })
  ...
  async toggleFollow(
    ...
    return firstValueFrom(
      this.communityClient.send("community.toggleFollow", {
        followerId: user.id,
        followingId: userId,
      }),
    );
  ...
  @ApiOperation({
    summary: "내 팔로우 여부 조회",
    description: "로그인한 사용자가 해당 사용자를 팔로우 중인지 조회합니다.",
  })
  ...
  async isFollowing(
    ...
    const following = await firstValueFrom(
      this.communityClient.send("community.isFollowing", {
        followerId: user.id,
        followingId: userId,
      }),
    );
  ...
  @ApiOperation({
    summary: "팔로워 목록 조회",
    description: "해당 사용자를 팔로우하는 사용자 목록을 조회합니다.",
  })
  @ApiParam({ name: "userId", description: "대상 사용자 ID" })
  @ApiQuery({ name: "page", required: false, description: "페이지 번호 (기본값 1)" })
  @ApiQuery({ name: "limit", required: false, description: "페이지당 항목 수 (기본값 20)" })
  ...
  @ApiOperation({
    summary: "팔로잉 목록 조회",
    description: "해당 사용자가 팔로우하는 사용자 목록을 조회합니다.",
  })
  @ApiParam({ name: "userId", description: "대상 사용자 ID" })
  @ApiQuery({ name: "page", required: false, description: "페이지 번호 (기본값 1)" })
  @ApiQuery({ name: "limit", required: false, description: "페이지당 항목 수 (기본값 20)" })
  ...
  @ApiOperation({
    ...
    description: "팔로워 수, 팔로잉 수, 받은 뱃지 수 등 프로필에 표시되는 통계를 조회합니다.",
  })
  @ApiParam({ name: "userId", description: "대상 사용자 ID" })
  @ApiResponse({ status: 200, description: "통계 정보 반환" })
  async getUserStats(@Param("userId", ParseUUIDPipe) userId: string) {
    return firstValueFrom(this.communityClient.send("community.getUserStats", { userId }));
  }
}
