...
export class AuthProxyController {
  ...
  @ApiResponse({ status: 201, description: "회원가입 성공 — 사용자 정보와 액세스/리프레시 토큰 반환" })
  ...
  @ApiResponse({ status: 200, description: "로그인 성공 — 사용자 정보와 액세스/리프레시 토큰 반환" })
  ...
  @ApiOperation({
    summary: "토큰 갱신",
    description: "만료된 액세스 토큰을 리프레시 토큰으로 재발급합니다. 리프레시 토큰 자체도 함께 갱신됩니다(로테이션).",
  })
  ...
  @ApiBearerAuth()
  @ApiOperation({ summary: "현재 로그인한 사용자 정보 조회", description: "Authorization 헤더의 액세스 토큰으로 본인 프로필 정보를 조회합니다." })
  ...
  @ApiOperation({
    ...
    description: "브라우저를 Google 로그인 동의 화면으로 리다이렉트합니다. GOOGLE_CLIENT_ID/SECRET 미설정 시 비활성화됩니다.",
  })
  ...
  @ApiOperation({
    ...
    description: "브라우저를 GitHub 로그인 동의 화면으로 리다이렉트합니다. GITHUB_CLIENT_ID/SECRET 미설정 시 비활성화됩니다.",
  })
