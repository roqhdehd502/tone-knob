...
export class CreateAiTabJobDto {
  ...
  @ApiPropertyOptional({
    description: "난이도",
    enum: TabDifficulty,
    example: TabDifficulty.INTERMEDIATE,
  })
  @IsOptional()
...
export class CreateAudioExtractionJobDto {
  @ApiProperty({
    description: "추출할 오디오 파일의 URL (Storage 업로드 후 받은 URL)",
    example: "https://xxxx.supabase.co/storage/v1/object/public/media/song.mp3",
  })
  @IsUrl()
  audioUrl: string;
  ...
  @ApiPropertyOptional({
    description: "튜닝 (예: standard, drop-d, half-step-down)",
    example: "standard",
    maxLength: 50,
  })
  @IsOptional()
