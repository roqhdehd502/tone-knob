...
export class ReviewProxyController {
  ...
  @ApiOperation({
    summary: "타브 리뷰 작성",
    description: "사용자당 타브 하나에 리뷰 1개만 작성할 수 있습니다(이미 작성한 경우 409).",
  })
  ...
  @ApiOperation({
    summary: "타브 리뷰 목록 조회",
    description: "인증 없이 누구나 조회 가능합니다.",
  })
  @ApiParam({ name: "tabId", description: "타브 ID" })
  ...
  async removeReview(
    ...
    await firstValueFrom(this.communityClient.send("review.remove", { reviewId, userId: user.id }));
  ...
  @ApiOperation({
    summary: "내 리뷰 조회",
    description: "로그인한 사용자가 해당 타브에 작성한 본인 리뷰를 조회합니다(없으면 null).",
  })
  @ApiParam({ name: "tabId", description: "타브 ID" })
