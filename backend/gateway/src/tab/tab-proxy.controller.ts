...
export class TabProxyController {
  ...
  @ApiOperation({
    summary: "타브 생성",
    description:
      "로그인한 사용자 명의로 새 타브를 생성합니다. 무료 플랜은 월 3회 생성 제한이 있습니다(subscription-svc에서 검증).",
  })
  ...
  async create(@CurrentUser() user: RequestUser, @Body() dto: CreateTabDto) {
    return firstValueFrom(
      this.tabClient
        .send("tabs.create", { userId: user.id, dto })
  ...
  @ApiOperation({
    summary: "공개 타브 목록 조회",
    description:
      "인증 없이 누구나 조회 가능한 공개(isPublic=true) 타브 목록입니다. 제목/아티스트 검색 및 특정 작성자 필터링을 지원합니다.",
  })
  ...
  @ApiOperation({
    summary: "내 타브 목록 조회",
    description: "로그인한 사용자가 작성한 모든 타브를 비공개 여부와 관계없이 조회합니다.",
  })
  @ApiQuery({ name: "page", required: false, type: Number, description: "페이지 번호 (기본값 1)" })
  @ApiQuery({
    name: "limit",
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: "팔로우 기반 피드 조회",
    description: "내가 팔로우한 사용자들이 공개한 타브를 최신순으로 조회합니다.",
  })
  @ApiQuery({ name: "page", required: false, type: Number, description: "페이지 번호 (기본값 1)" })
  @ApiQuery({
    name: "limit",
  ...
  @ApiOperation({
    summary: "타브 상세 조회",
    description: "비공개 타브는 작성자 본인만 조회할 수 있습니다(로그인 시 토큰 전달 권장).",
  })
  ...
  async findOne(@Param("id") id: string, @CurrentUser() user?: RequestUser) {
    return firstValueFrom(
      this.tabClient
        .send("tabs.findOne", { id, requestUserId: user?.id })
        .pipe(catchError((err) => throwError(() => err))),
    );
  }
  ...
  @ApiOperation({
    summary: "타브 수정",
    description: "작성자 본인만 수정할 수 있습니다. 전달한 필드만 갱신됩니다(부분 수정).",
  })
  @ApiParam({ name: "id", description: "타브 ID" })
  ...
  async update(
    ...
    return firstValueFrom(
      this.tabClient
        .send("tabs.update", { id, userId: user.id, dto })
        .pipe(catchError((err) => throwError(() => err))),
    );
  }
  ...
  @ApiOperation({
    summary: "타브 삭제",
    description: "작성자 본인만 삭제할 수 있습니다. 연결된 버전 기록도 함께 삭제됩니다.",
  })
  @ApiParam({ name: "id", description: "타브 ID" })
  ...
  async remove(@Param("id") id: string, @CurrentUser() user: RequestUser) {
    return firstValueFrom(
      this.tabClient
        .send("tabs.remove", { id, userId: user.id })
        .pipe(catchError((err) => throwError(() => err))),
    );
  }
  ...
  @ApiOperation({
    summary: "타브 포크",
    description:
      "다른 사람의 공개 타브를 복제해 내 소유의 새 타브로 만듭니다(원본 내용 그대로 복사, 독립적으로 수정 가능).",
  })
  ...
  async fork(@Param("id") id: string, @CurrentUser() user: RequestUser) {
    return firstValueFrom(
      this.tabClient
        .send("tabs.fork", { id, userId: user.id })
        .pipe(catchError((err) => throwError(() => err))),
    );
  }
  ...
  @Get(":id/versions")
  @ApiOperation({
    summary: "타브 버전 히스토리 조회",
    description: "수정될 때마다 자동 저장되는 버전 기록을 시간순으로 조회합니다.",
  })
  @ApiParam({ name: "id", description: "타브 ID" })
  ...
  async getVersions(@Param("id") id: string, @CurrentUser() user?: RequestUser) {
    return firstValueFrom(
      this.tabClient
        .send("tabs.getVersions", { tabId: id, requestUserId: user?.id })
        .pipe(catchError((err) => throwError(() => err))),
    );
  }
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: "타브 공개/비공개 토글",
    description: "작성자 본인만 가능합니다. 호출마다 현재 상태의 반대로 전환됩니다.",
  })
  @ApiParam({ name: "id", description: "타브 ID" })
  ...
  async togglePublish(@Param("id") id: string, @CurrentUser() user: RequestUser) {
    return firstValueFrom(
      this.tabClient
        .send("tabs.togglePublish", { id, userId: user.id })
        .pipe(catchError((err) => throwError(() => err))),
    );
  }
}
