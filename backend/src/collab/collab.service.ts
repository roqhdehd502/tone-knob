...
export class CollabService {
  ...
  async getSession(tabId: string): Promise<CollabSession> {
    ...
    if (!session) {
      throw new NotFoundException('협업 세션을 찾을 수 없습니다');
    }
    return session;
  }
  ...
  async applyOperation(
    ...
    // 이미 최신 리비전보다 오래된 요청이면 충돌 처리
    if (clientRevision < session.revision) {
      // 누락된 ops 가져와서 OT 변환 (단순화: 페이로드를 그대로 수락)
  ...
  async leaveSession(tabId: string, userId: string): Promise<void> {
    const session = await this.sessionRepository.findOne({ where: { tabId } });
    if (!session) return;
    session.activeUserIds = (session.activeUserIds || []).filter(
      (id) => id !== userId,
    );
    await this.sessionRepository.save(session);
  }
