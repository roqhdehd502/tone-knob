...
export class CollabGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  // socketId -> { userId, tabId }
  private readonly connections = new Map<
    string,
    { userId: string; tabId: string }
  >();
  ...
  async handleDisconnect(client: AuthSocket) {
    ...
    if (conn) {
      ...
      this.server.to(`tab:${conn.tabId}`).emit('user:left', {
        userId: conn.userId,
      });
  ...
  async handleJoin(
    ...
    const session = await this.collabService.joinSession(
      data.tabId,
      client.userId,
    );
    ...
    // 다른 참가자에게 입장 알림
    client.to(`tab:${data.tabId}`).emit('user:joined', {
      userId: client.userId,
    });
  }
  ...
  handleCursor(
    ...
    client.to(`tab:${data.tabId}`).emit('cursor:update', {
      userId: client.userId,
  ...
  async handleSync(
    ...
    if (!client.userId) return;
    const ops = await this.collabService.getOperationsSince(
      data.tabId,
      data.sinceRevision,
    );
