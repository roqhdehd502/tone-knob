...
export class MarketplaceService {
  ...
  async setPrice(tabId: string, userId: string, price: number): Promise<Tab> {
    ...
    if (tab.userId !== userId) {
      throw new ForbiddenException('이 타브의 가격을 설정할 권한이 없습니다');
    }
    tab.price = price;
  ...
  async purchase(tabId: string, buyerId: string): Promise<TabPurchase> {
    ...
    if (!tab) throw new NotFoundException('타브를 찾을 수 없습니다');
    if (!tab.isPublic) {
      throw new ForbiddenException('비공개 타브는 구매할 수 없습니다');
    }
    if (tab.price <= 0) {
      throw new ForbiddenException('무료 타브입니다');
    }
    if (tab.userId === buyerId) {
      throw new ForbiddenException('자신의 타브는 구매할 수 없습니다');
    }
    ...
    if (existing) {
      throw new ConflictException('이미 구매한 타브입니다');
    }
