...
export function meta() {
  return [{ title: "Knob 재화 관리 - Tone Knob Admin" }];
}
...
export async function action({ request }: Route.ActionArgs) {
  ...
  const type =
    amount > 0 ? "earn_community_activity" : "spend_premium_feature";
...
function txTypeLabel(t: string) {
  const map: Record<string, string> = {
    earn_tab_created: "타브 생성",
    earn_jam_participated: "합주 참여",
    earn_community_activity: "커뮤니티",
    earn_daily_login: "일일 로그인",
    earn_tab_sale: "탭 판매",
    spend_tab_purchase: "탭 구매",
    spend_premium_feature: "프리미엄",
  };
  return map[t] ?? t;
}

function txVariant(t: string) {
  return t.startsWith("earn") ? ("success" as const) : ("warning" as const);
}

export default function KnobPage({ loaderData }: Route.ComponentProps) {
  const { admin, topUsers, transactions, txTotal, page, typeFilter, txTotalPages, error } = loaderData;
  ...
  return (
    <>
      <Topbar adminName={admin.adminName} adminEmail={admin.adminEmail} title="Knob 재화 관리" />
        ...
        <div className="mx-auto max-w-7xl space-y-6">
          ...
          <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              <h2 className="mb-4 text-sm font-semibold text-slate-900">수동 Knob 지급/차감</h2>
              {error === "user_not_found" && (
                <p className="mb-3 rounded-lg bg-red-50 px-3 py-2 text-xs text-red-600">해당 이메일의 유저를 찾을 수 없습니다.</p>
              )}
              <form method="post" className="space-y-3">
                ...
                <div>
                  <label className="mb-1 block text-xs font-medium text-slate-600">사유</label>
                  <input
                    name="reason"
                    placeholder="운영 보정"
                    className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
            ...
            <div className="lg:col-span-2 rounded-xl border border-slate-200 bg-white shadow-sm">
              <div className="border-b border-slate-100 px-4 py-3 sm:px-5">
                <h2 className="text-sm font-semibold text-slate-900">잔액 상위 10명</h2>
              </div>
              <div className="divide-y divide-slate-50">
                {topUsers.length === 0 ? (
                  ...
                  topUsers.map((u, idx) => (
                    <div key={u.id} className="flex items-center justify-between px-4 py-2.5 sm:px-5">
          ...
          <div>
            <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm text-slate-500">
                트랜잭션 총 <span className="font-semibold text-slate-900">{txTotal.toLocaleString()}</span>건
              </p>
              <div className="flex flex-wrap gap-1">
                {TX_TYPE_OPTIONS.map((v, i) => (
                  <a
                    key={v}
              ...
              <table className="w-full min-w-[580px] text-sm">
                <thead>
                  <tr className="border-b border-slate-100">
                    ...
                    <th className="px-4 py-3.5 text-right text-xs font-semibold uppercase tracking-wide text-slate-500 sm:px-5">잔액 후</th>
                    <th className="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-slate-500 sm:px-5">설명</th>
                ...
                <tbody className="divide-y divide-slate-50">
                  {transactions.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="px-5 py-8 text-center text-slate-400">트랜잭션이 없습니다.</td>
                    </tr>
                  ) : (
                    transactions.map((tx) => (
                      <tr key={tx.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-4 py-3 text-right font-medium sm:px-5">
                          <span className={tx.type.startsWith("earn") ? "text-emerald-600" : "text-red-500"}>
                            {tx.type.startsWith("earn") ? "+" : "-"}{tx.amount.toLocaleString()}
                          </span>
                        </td>
            ...
            {txTotalPages > 1 && (
              <div className="mt-3 flex items-center justify-center gap-1">
                ...
                <span className="px-3 py-1.5 text-sm text-slate-500">{page} / {txTotalPages}</span>
                {page < txTotalPages && (
