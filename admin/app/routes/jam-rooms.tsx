...
export function meta() {
  return [{ title: "합주방 관리 - Tone Knob Admin" }];
}
...
export async function action({ request }: Route.ActionArgs) {
  ...
  await supabase
    .from("jam_rooms")
    .update({ isActive: false, currentParticipants: 0 })
    .eq("id", id);
...
export default function JamRoomsPage({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <>
      <Topbar adminName={admin.adminName} adminEmail={admin.adminEmail} title="합주방 관리" />
            ...
            <table className="w-full min-w-[620px] text-sm">
              <thead>
                <tr className="border-b border-slate-100">
                  <th className="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-slate-500 sm:px-5">방 이름</th>
                  <th className="px-4 py-3.5 text-center text-xs font-semibold uppercase tracking-wide text-slate-500 sm:px-5">인원</th>
                  <th className="px-4 py-3.5 text-center text-xs font-semibold uppercase tracking-wide text-slate-500 sm:px-5">BPM</th>
                  <th className="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-slate-500 sm:px-5">상태</th>
                  <th className="px-4 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-slate-500 sm:px-5">공개</th>
              ...
              <tbody className="divide-y divide-slate-50">
                {rooms.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-5 py-8 text-center text-slate-400">합주방이 없습니다.</td>
                  </tr>
                ) : (
                  rooms.map((room) => (
                    <tr key={room.id} className="hover:bg-slate-50">
                      <td className="max-w-[180px] truncate px-4 py-3 font-medium text-slate-900 sm:max-w-[220px] sm:px-5">{room.name}</td>
                      ...
                      <td className="px-4 py-3 sm:px-5">
                        {room.isActive && (
                          <form
                            method="post"
                            onSubmit={(e) => {
                              if (!confirm(`"${room.name}" 합주방을 강제 종료하시겠습니까?`)) {
                                e.preventDefault();
                              }
                            }}
                          >
